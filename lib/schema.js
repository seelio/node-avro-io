...
var _parseNamedType = function(schema, namespace) {
    if (_.contains(PRIMITIVE_TYPES, schema)) {
        ...
        throw new AvroErrors.InvalidSchemaError(
                'unknown type name: %s; known type names are ',
...
function makeFullyQualifiedTypeName(schema, namespace) {
    ...
    if (_.isString(schema)) {
        ...
        throw new AvroErrors.InvalidSchemaError(
                'unable to determine fully qualified type name from schema ' +
                '%s in namespace %s',
                JSON.stringify(schema),
                namespace
        );
    ...
    if (!_.isString(typeName)) {
        throw new AvroErrors.InvalidSchemaError(
                'unable to determine type name from schema %s in namespace %s',
                JSON.stringify(schema),
                namespace
        );
    }
...
_.extend(Schema.prototype, {
    ...
    parse: function(schema, namespace) {
        ...
        if (_.isNull(schema) || _.isUndefined(schema)) {
            throw new AvroErrors.InvalidSchemaError(
                'schema is null, in parentSchema: %s',
            ...
            if (schema.type === 'record') {
                if (!_.has(schema, 'fields')) {
                    throw new AvroErrors.InvalidSchemaError(
                        'record must specify "fields", got %s',
                        JSON.stringify(schema)
                    );
                } else if (!_.has(schema, 'name')) {
                    throw new AvroErrors.InvalidSchemaError(
                            'record must specify "name", got %s',
                            JSON.stringify(schema)
                    );
                } else {
                ...
                if (_.has(schema, 'symbols')) {
                    ...
                    throw new AvroErrors.InvalidSchemaError(
                            'enum must specify "symbols", got %s',
                            JSON.stringify(schema)
                    );
                }
                ...
                if (_.has(schema, 'items')) {
                    return new ArraySchema(
                            this.parse(schema.items, namespace),
                            namespace
                    );
                } else {
                    throw new AvroErrors.InvalidSchemaError(
                            'array must specify "items", got %s',
                            JSON.stringify(schema)
                    );
                }
                ...
                if (_.has(schema, 'values')) {
                    ...
                    throw new AvroErrors.InvalidSchemaError(
                            'map must specify "values" schema, got %s',
                            JSON.stringify(schema)
                    );
                }
            ...
            if (_.isEmpty(schema)) {
                throw new AvroErrors.InvalidSchemaError(
                        'unions must have at least 1 branch'
                );
            }
            ...
            throw new AvroErrors.InvalidSchemaError(
                    'unexpected Javascript type for schema: ' + (typeof schema)
            );
        }
    },
    ...
    validateAndThrow: function(schema, datum){
        ...
        switch (schema) {
            case 'null':
                if (!_.isNull(datum)) {
                    throw new AvroErrors.DataValidationError(
                            'Data [%j] is not null',
                            datum
                    );
                }
            ...
            case 'double':
                if (!_.isNumber(datum) || datum === null) {
                    throw new AvroErrors.DataValidationError(
                            'Data [%j] is not a number or not defined',
                            datum
                    );
                }
                break;
            case 'int':
            case 'long':
                if ((!_.isNumber(datum) && !Long.isLong(datum)) ||
                        datum === null) {
                    throw new AvroErrors.DataValidationError(
                            'Data [%j] is not a number or not defined',
                            datum
            ...
            case 'enum':
                if (datum === null || _.indexOf(this.symbols, datum) === -1) {
                    throw new AvroErrors.DataValidationError(
                            'Data [%j] not a valid enum value. List of ' +
                            'values [%j]',
                            datum,
                            this.symbols
                    );
            ...
            case 'record':
                ...
                if (intersect.length < dFields.length) {
                    throw new AvroErrors.DataValidationError(
                            'Data [%j] has extra fields not in schema. data ' +
...
function PrimitiveSchema(type) {
    ...
    if (!_.isString(type)) {
        throw new AvroErrors.InvalidSchemaError(
                'Primitive type name must be a string'
        );
    }
    if (!_.contains(PRIMITIVE_TYPES, type)) {
        throw new AvroErrors.InvalidSchemaError(
                'Primitive type must be one of: %s; got %s',
                JSON.stringify(PRIMITIVE_TYPES),
                type
        );
    }
...
function Field(name, type, defaultValue) {
    ...
    if (!(type instanceof Schema)) {
        throw new AvroErrors.InvalidSchemaError(
                'Field type must be a Schema object'
        );
    }
...
function RecordSchema(name, namespace, fields) {
    ...
    if (!_.isNull(namespace) &&
            !_.isUndefined(namespace) &&
            !_.isString(namespace)) {
        throw new AvroErrors.InvalidSchemaError(
                'Record namespace must be string or null'
        );
    }
...
function ArraySchema(items) {
    if (_.isNull(items) || _.isUndefined(items)) {
        throw new AvroErrors.InvalidSchemaError(
                'Array "items" schema should not be null or undefined'
        );
    }
...
function EnumSchema(symbols) {
    if (!_.isArray(symbols)) {
        throw new AvroErrors.InvalidSchemaError(
                'Enum must have array of symbols, got %s',
                JSON.stringify(symbols)
        );
    }
    if (!_.all(symbols, function(symbol) { return _.isString(symbol); })) {
        throw new AvroErrors.InvalidSchemaError(
                'Enum symbols must be strings, got %s',
                JSON.stringify(symbols)
        );
    }
