    ...
    skip: function(size) {
        if ((this._readOffset + size) >= 0 && size <= this.remainingBytes)
            ...
            throw new AvroErrors.BlockError(
                'tried to skip(%d) outsite of the block(%d) at %d',
                size,
                this.remainingBytes,
                this._readOffset);
    },
    ...
    read: function(size) {
        ...
        if (size > this.remainingBytes) {
            throw new AvroErrors.BlockDelayReadError(
                'tried to read %d bytes past the amount written to the block ' +
                'with remaining bytes %d at read offset %d',
                size,
                this.remainingBytes,
                this._readOffset
            );
            ...
            throw new AvroErrors.BlockError(
                'tried to read %d bytes outside of the buffer(%d) at read ' +
                'offset %d',
                size,
                this._buffer.length,
                this._readOffset
    ...
    _readHeader: function() {
        ...
        if (header instanceof Error) {
            ...
            this.emit('error', new AvroErrors.FileError(
                        "Not an avro file, header was %j", header
            ));
        }
        try {
